...
- Use `--markdown` when you want a lightweight formatted reply and you accept that the shortcut will rewrite the content before sending.
...
3. Rewrites the Markdown for Feishu post rendering
...
- Consecutive headings may get `<br>` inserted.
- Tables may get extra `<br>` spacing inserted.
- Code blocks are preserved, but may be wrapped with extra `<br>`.
...
This is better for quick readable formatting, but the final payload is rewritten before sending.
...
| `--markdown <string>` | One content option | Convenience Markdown input. Internally converted to `post` JSON with Feishu-specific rewriting |
