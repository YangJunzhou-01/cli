...
- Use `--markdown` when you want basic Markdown-style rendering and you accept that the shortcut will **rewrite** the content before sending.
...
3. Rewrites the Markdown for better Feishu post rendering
...
- Consecutive headings may get `<br>` inserted between them.
- Tables may get extra `<br>` spacing inserted.
- Code blocks are preserved, but the wrapper may add `<br>` around them.
...
This is better for lightweight readable formatting, but the final content may not match the source text byte-for-byte because the shortcut rewrites it.
...
| `--markdown <string>` | One content option | Convenience Markdown input. Internally converted to `post` JSON with Feishu-specific rewriting; not full Markdown passthrough |
