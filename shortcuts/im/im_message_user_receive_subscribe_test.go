...
func TestMessageUserReceiveSubscribeValidate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if problem, ok := errs.ProblemOf(err); !ok || problem.Category != errs.CategoryValidation || problem.Subtype != errs.SubtypeInvalidArgument {
				t.Fatalf("Validate() should return typed validation error, got %T: %v", err, err)
			}
		})
	}
}

func TestMessageUserReceiveSubscribeValidate_ParamMetadata(t *testing.T) {
	tests := []struct {
		name      string
		flags     map[string]string
		wantParam string
